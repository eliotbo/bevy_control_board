...
layout(set = 2, binding = 2) uniform KnobShader_zoom_time{
    vec2 zoom_time;
};
...
float Function(float x)
{
    x = x * 8.;
    x -= zoom_time.y * .1;
    return abs(4.0 * sin(x * sin(zoom_time.y * .2)) * sin(x + cos(zoom_time.y * zoom_time.y * .01)) * .25) + .1;
}
...
void main( )
{
    ...
    float deltay = deltax * dy;


    // x0 = uv.x
    // x1 = 
    float x1 = uv.x - deltax ;
    float y1_est = uv.y - deltay;
    ...
    // float left_dydx1 = (Function(x1)-Function(x1-epsilon))/epsilon;
    // float right_dydx1 = (Function(x1+epsilon)-Function(x1))/epsilon;

    float dydx1 = (Function(x1+0.5*epsilon)-Function(x1-0.5*epsilon))/epsilon;

    float y1_real = Function(x1);
    ...
    float zz = zoom_time.x;

    //// compare the actual height of the function at x1
    // float x1 = uv.x - dir * abs(normal.x) * a ;
    // if (abs(y1_est - y1_real) > 0.2) {
    //     zz = 1;
    // }

    // TODO: there is a gap at the exact point where the derivative is discontinuous
    // because the following if statement accidentally catches it. Add a condition?
    if (abs(dydx1 - dy) > 2.0) {
        zz = 1;
    }
